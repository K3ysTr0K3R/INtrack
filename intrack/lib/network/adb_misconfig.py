    ...
    for port in ports:
        try:
            ...
            if product_name or product_model or product_device:
                print_colour(f"[+] Android Debug Bridge: {ip}:{port} [Product Name: {product_name.group(1) if product_name else 'N/A'}] [Product Model: {product_model.group(1) if product_model else 'N/A'}] [Product Device: {product_device.group(1) if product_device else 'N/A'}]")
                return True
        except Exception:
            return False
        finally:
